...
const Customer = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/rest/admin/customers-auths`, config)
  ...
  const renderSelfTD = (selfData, key, index, mainIndex) => {
    ...
    const dataIndex = data?.length && data?.map((item, index) => index > 0 ? index * 14 : index)
  ...
  return (
    <>
      <Button variant="contained" onClick={() => navigate("/did-list")}>Go to did list page</Button>
          ...
          <TableHead sx={{ fontWeight: "700" }}>
            ...
            <TableRow>
              ...
              {subHeading?.length && subHeading?.map((heading, index) => {
                return (
                  <TableCell align='center' key={index} colSpan={index >= attributeHeading?.length ? 2 : 1} >{heading}</TableCell>
                )
              })}
            </TableRow>
            <TableRow>
